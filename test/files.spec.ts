...
describe("document file snapshots", () => {
  it("migrates old filenames to safe paths without rewriting source keys or names", async () => {
    ...
    for (let i = 0; i < names.length; i++)
      await env.DB.prepare(
        "INSERT INTO migration_document_version VALUES (?, 1, ?, 'md', 'text/markdown', ?)",
      )
    ...
    const { results } = await env.DB.prepare(
      "SELECT * FROM migration_document_file ORDER BY document_id",
    ).all<{
      path: string;
      filename: string;
      r2_key: string;
  ...
  it("merges paths, preserves original bytes, deletes explicitly, and restores the whole set", async () => {
    ...
    await addVersion(env, await current(id), now(), {
      title: null,
      files: [],
      delete_paths: ["adr/001.md"],
    });
    expect((await listVersionFiles(env.DB, id, 3)).map((entry) => entry.path)).toEqual([
      "overview.md",
      "adr/002.md",
    ]);
    await rollbackDocument(env, await current(id), 1, now());
    expect((await current(id)).version).toBe(1);
    expect((await listVersionFiles(env.DB, id, 1)).map((entry) => entry.path)).toEqual([
      "overview.md",
      "adr/001.md",
    ]);
  });
  ...
  it("keeps other files when a legacy client updates one source", async () => {
    ...
    expect((await listVersionFiles(env.DB, id, 2)).map((entry) => entry.path)).toEqual([
      "overview.md",
      "adr/001.md",
    ]);
  });
  ...
  it("rejects stale writers without changing the published snapshot", async () => {
    ...
    await expect(
      addVersion(env, stale, now(), { title: null, files: [file("lost.md")] }),
  ...
  it.each(["create", "update"] as const)(
    "drains late writes before cleaning up a failed %s",
    async (operation) => {
  ...
  it("rejects empty snapshots, duplicate paths, and aggregate oversized uploads", async () => {
    ...
    await expect(
      addVersion(env, doc, now(), {
        title: null,
        files: [],
        delete_paths: ["overview.md", "adr/001.md"],
      }),
    ).rejects.toThrow("at least one file");
    await expect(
      addVersion(env, doc, now(), { title: null, files: [file("x.md"), file("x.md")] }),
    ).rejects.toThrow("Duplicate");
    await expect(
      addVersion(env, doc, now(), {
        title: null,
        files: [
          file("x.md", "x".repeat(MAX_BYTES / 2 + 1)),
          file("y.md", "y".repeat(MAX_BYTES / 2)),
        ],
      }),
  ...
  it("supports mixed multipart files, path-specific content, and delete-only updates", async () => {
    ...
    const content = await SELF.fetch(
      `${OWNER_BASE}/api/documents/${id}/content?file=site%2Findex.html&format=raw`,
    );
