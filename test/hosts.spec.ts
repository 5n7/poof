...
async function fetchWithHosts(
  url: string,
  hosts: { MCP_HOST?: string; OWNER_HOST?: string },
  init?: RequestInit,
) {
...
describe("semantically equal host settings fail closed", () => {
  ...
  it("still routes a configured non-default port", async () => {
    ...
    expect((await fetchWithHosts("https://mcp.example:8443/mcp", hosts, MCP_CALL)).status).toBe(
      200,
    );
