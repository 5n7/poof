...
describe("MCP endpoint", () => {
  ...
  it("annotates every tool with its behavior hints", async () => {
    const { result } = await rpc<{ tools: { annotations?: ToolAnnotations; name: string }[] }>(
      "tools/list",
    );
    ...
    for (const name of TOOL_NAMES)
      expect(annotationsOf(name), name).toMatchObject({ openWorldHint: false });
  });
  ...
  it("leaves kind optional so handlers can infer file types", async () => {
    const { result } = await rpc<{
      tools: {
        inputSchema: { properties: Record<string, { default?: string; description: string }> };
        name: string;
      }[];
    }>("tools/list");
    const kindOf = (name: string) =>
      result.tools.find((t) => t.name === name)!.inputSchema.properties.kind;
  ...
  it("builds the server per request, so each caller gets its own origin", async () => {
    ...
    const frame = (await res.text()).split("\n").find((line) => line.startsWith("data:"))!;
    const text = (JSON.parse(frame.slice("data:".length)) as RpcResponse<ToolResult>).result
      .content[0].text;
...
describe("MCP owner-TTL gating", () => {
  ...
  it("still lets rm delete an expired document", async () => {
    ...
    const row = await env.DB.prepare("SELECT COUNT(*) AS n FROM document WHERE id = ?")
      .bind(id)
      .first<{ n: number }>();
    expect(row!.n).toBe(0);
  });
...
describe("MCP tool errors", () => {
  it("reports an unknown id as a tool error, not a transport failure", async () => {
    ...
    for (const name of ["rm", "share", "update", "versions"]) {
      const args =
        name === "update" ? { content: "# x", id: "does_not_exist" } : { id: "does_not_exist" };
  ...
  it("reports an unknown version and an unknown share token", async () => {
    ...
    expect(await callToolExpectingError("rollback", { id, version: 99 })).toContain(
      "No version 99",
    );
...
describe("MCP file sets", () => {
  it("creates mixed files, merges by path, deletes explicitly, and restores a snapshot", async () => {
    ...
    expect(await callTool("cat", { id, file: "adr/001.html", raw: true })).toBe(
      "<h1>Decision</h1>",
    );
    expect(await callTool("cat", { id, file: "assets/logo.png" })).toContain(
      "file=assets%2Flogo.png",
    );
    ...
    expect(await callToolExpectingError("cat", { id, file: "assets/logo.png" })).toContain(
      "No file",
    );
  ...
  it("rejects ambiguous, empty, duplicate, unsafe, or undecodable file inputs", async () => {
    ...
    ).toContain("either content or files");
    expect(await callToolExpectingError("push", { files: [] })).toContain("At least one file");
    expect(
      await callToolExpectingError("push", { files: [{ path: "../a.md", content: "x" }] }),
    ).toContain("path");
    expect(
      await callToolExpectingError("push", {
        files: [
    ...
    expect(
      await callToolExpectingError("push", {
        files: [{ path: "a.png", content: "!", encoding: "base64" }],
      }),
    ).toContain("base64");
  });
});
...
describe("MCP title tools", () => {
  it("renames metadata without creating a version and rejects stale title expectations", async () => {
    ...
    const current = await env.DB.prepare("SELECT title, current_version FROM document WHERE id = ?")
      .bind(id)
      .first();
    expect(current).toMatchObject({ title: "New title", current_version: 1 });
    const history = await env.DB.prepare(
      "SELECT COUNT(*) AS count FROM document_version WHERE document_id = ?",
    )
      .bind(id)
      .first();
  ...
  it("uses the ls state token to rename binary documents with joiners in their titles", async () => {
    for (const [suffix, title] of [
      ...
      const renamed = JSON.parse(
        await callTool("rename", { id, title: "Chosen title", expected_state }),
      );
  ...
  it("does not rename a document when there is no readable text to suggest from", async () => {
    ...
    const before = await env.DB.prepare("SELECT title, updated_at FROM document WHERE id = ?")
      .bind(id)
      .first();
    expect(await callToolExpectingError("suggest_title", { id })).toContain("no readable text");
    const after = await env.DB.prepare("SELECT title, updated_at FROM document WHERE id = ?")
      .bind(id)
      .first();
