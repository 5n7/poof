...
async function expectation(id: string) {
  return {
    expected_state: await documentTitleState({ id, title: "Original", current_version: 1 }),
  };
}
async function document(
  files = [{ path: "notes.md", kind: "md" as const, source: "# Original source" }],
) {
...
describe("document title changes", () => {
  ...
  it("checks expiry atomically when it passes between the read and the write", async () => {
    const id = await createDocument(env, 0, {
      title: "Original",
      expires_at: 2,
      kind: "md",
      source: "body",
    ...
    await expect(renameDocument(env, id, "Too late", expected)).rejects.toMatchObject({
      status: 404,
    });
  ...
  it("rejects stale rename and content upload snapshots without adding history", async () => {
    ...
    await expect(
      addVersion(env, original, 200, { kind: "md", source: "new", title: null }),
  ...
  it("uses the live name in Markdown viewers and keeps pinned version titles after rollback", async () => {
    ...
    expect(await (await SELF.fetch(`${OWNER_BASE}/raw/${live}`)).text()).toContain(
      "<title>Live name</title>",
    );
    expect(await (await SELF.fetch(`${OWNER_BASE}/raw/${pinned}`)).text()).toContain(
...
describe("title suggestions", () => {
    ...
    const id = await createDocument(env, 100, {
      ...
      files: [
        ...
        { path: "one.md", kind: "md", source: "First readable note " + "a".repeat(10000) },
        {
          path: "two.html",
          kind: "html",
          source: "<script>script-secret</script><p>Second readable note</p>",
        },
      ],
    });
    const expected = await expectation(id);
    const ai = vi.spyOn(titleGenerator, "generateAiTitle").mockResolvedValue("Suggested");
    expect(await suggestDocumentTitle(env, id, expected)).toEqual({
      title: "Suggested",
      ...expected,
    });
  ...
  it("keeps language detection free of English filename labels and rejects unusable candidates", async () => {
    const id = await document([
      { path: "english-filename.md", kind: "md", source: "明日の会議は十時から" },
    ]);
    const expected = await expectation(id);
  ...
  it("detects rename, content changes, deletion and expiry during inference", async () => {
    for (const change of ["rename", "content", "delete", "expire"] as const) {
      ...
      vi.spyOn(titleGenerator, "generateAiTitle").mockImplementation(async () => {
        ...
        if (change === "delete")
          await env.DB.prepare("DELETE FROM document WHERE id = ?").bind(id).run();
        if (change === "expire")
  ...
  it("does not call AI for binary-only or empty documents and reports AI failure", async () => {
    ...
    expect(ai).not.toHaveBeenCalled();
    const id = await document();
    await expect(suggestDocumentTitle(env, id, await expectation(id))).rejects.toMatchObject({
      status: 503,
    });
  });
});
...
describe("title API", () => {
  ...
  it("preserves joined Unicode titles in state tokens and binds tokens to the document", async () => {
    ...
    await expect(
      renameDocument(env, otherId, "Other", { expected_state: renamed.state }),
    ).rejects.toMatchObject({
      status: 409,
    });
  ...
  it("bounds chunked and oversized JSON requests before parsing", async () => {
    ...
    const bytes = new TextEncoder().encode(
      JSON.stringify({ ...expected, title: "x".repeat(20000) }),
    );
  ...
  it("returns narrow metadata, validates JSON and snapshots, escapes names and updates shared viewers", async () => {
    ...
    for (const body of [[], null, {}, { ...expected, title: 5 }, { ...expected, title: "" }]) {
      expect(
        (await SELF.fetch(`${OWNER_BASE}/api/documents/${id}/title`, titleRequest(body))).status,
      ).toBe(400);
    }
    ...
    for (const path of [`/d/${id}`, `/v/${token}`, `/raw/${token}`, "/"]) {
      expect(await (await SELF.fetch(`${OWNER_BASE}${path}`)).text()).toContain(
    ...
    expect(
      (
        await SELF.fetch(
          `${OWNER_BASE}/api/documents/${id}/title`,
          titleRequest({ ...expected, title: "Stale" }),
        )
      ).status,
    ).toBe(409);
  });
  ...
  it("keeps missing, expired and unauthorized documents unavailable and rejects cross-origin writes", async () => {
    ...
    for (const method of ["PATCH", "POST"]) {
      ...
      expect((await SELF.fetch(`${OWNER_BASE}/api/documents/missing/${suffix}`, init)).status).toBe(
        404,
      );
      expect(
        (
          await fetchWorker(
            `${OWNER_BASE}/api/documents/${id}/${suffix}`,
            { DEV_DISABLE_ACCESS: "" },
            init,
          )
        ).status,
    ...
    expect(
      (
        await SELF.fetch(
          `${OWNER_BASE}/api/documents/${id}/title`,
          titleRequest({ ...expected, title: "Name" }),
        )
      ).status,
    ).toBe(404);
  });
