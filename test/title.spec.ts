...
describe("sanitizeAiTitle", () => {
  ...
  it("leaves a string that merely contains a Markdown link alone", () => {
    expect(sanitizeAiTitle("[a](http://x) and [b](http://y)")).toBe(
      "[a](http://x) and [b](http://y)",
    );
    expect(sanitizeAiTitle("see [Design Notes](http://x) here")).toBe(
      "see [Design Notes](http://x) here",
    );
  });
...
after five attempts instead of spinning forever.`;
  ...
  const EN_FILLER =
    "The deploy pipeline runs on every merge to main and publishes the worker. ".repeat(20);
...
still open after three weeks of back and forth with their support team.`;
    ...
    expect(
      looksJapanese(`${EN_FILLER}The account manager is トヨタ, contract renewed in April.`),
    ).toBe(false);
  });
...
付録 参考文献一覧`;
  ...
  it("does not classify kanji without kana as Japanese", () => {
    ...
    expect(
      looksJapanese("本季度销售报告显示，华东地区的增长速度超过预期，主要来自新客户的持续投入。"),
    ).toBe(false);
