...
describe("document file navigation", () => {
  ...
  it("encodes nested Unicode paths and preserves file/version selection", () => {
    expect(rawFileUrl("s_token", "adr/日本 語.md")).toBe(
      "/raw/s_token/adr/%E6%97%A5%E6%9C%AC%20%E8%AA%9E.md",
    );
  ...
  it("keeps query-bearing navigation native while bridging ordinary file links and fragments", () => {
    for (const query of ["?mode=print#details", "?format=raw", "?file=other.md&v=2#details"]) {
      const { preventDefault, postMessage } = clickFileLink(
        `${OWNER_BASE}/raw/s_navigation/adr/001.md${query}`,
      );
      expect(preventDefault).not.toHaveBeenCalled();
      expect(postMessage).not.toHaveBeenCalled();
    }
    const { preventDefault, postMessage } = clickFileLink(
      `${OWNER_BASE}/raw/s_navigation/adr/001.md#details`,
    );
    expect(preventDefault).toHaveBeenCalledOnce();
    expect(postMessage).toHaveBeenCalledWith(
      { type: "poof:file", path: "adr/001.md", hash: "#details" },
      "*",
    );
  });
  ...
  it("leaves whitespace-prefixed external URLs and external base descendants unchanged", async () => {
    ...
    const html = await linkDocumentFiles(new Response(source), "s_external", "index.html", [
      "index.html",
    ]).text();
  ...
  it("serves CSS and module scripts as source with CORS for opaque frames", async () => {
    const { token } = await seedFiles();
    const html = await (
      await SELF.fetch(`${OWNER_BASE}/raw/${token}/prototype/index.html?view=1`)
    ).text();
    ...
    const preview = await (
      await SELF.fetch(`${OWNER_BASE}/raw/${token}/assets/style.css?view=1`)
    ).text();
