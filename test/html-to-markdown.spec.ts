...
<tbody><tr><td>A | B</td><td><strong>2</strong><br>items</td></tr></tbody></table>`),
    ...
    expect(
      htmlToMarkdown("<table><tr><td>first row</td></tr><tr><td>second row</td></tr></table>"),
    ).toBe("|  |\n| --- |\n| first row |\n| second row |");
  });
  ...
  it.each([50, 1_500])(
    "keeps all cells in a jagged table of width %i without rectangular padding",
    (width) => {
  ...
  it("parses malformed HTML and safely encodes link destinations", () => {
    expect(
      htmlToMarkdown('<h1>Title</h1><p>One<p>Two <a href="/a(b) c" title="x > y">link</a>'),
  ...
  it("keeps code inside lists, blockquotes, and table cells valid", () => {
    ...
    expect(
      htmlToMarkdown(
        "<table><tr><th>Expression</th></tr><tr><td><code>a | b</code></td></tr></table>",
      ),
    ).toBe("| Expression |\n| --- |\n| `a \\| b` |");
  });
  ...
  it("omits embedded image payloads while keeping their descriptions", () => {
    expect(
      htmlToMarkdown('<p><img src="data:image/png;base64,verylongpayload" alt="Chart"></p>'),
    ).toBe("Chart");
    expect(htmlToMarkdown('<a href="data:text/plain;base64,verylongpayload">Attachment</a>')).toBe(
      "Attachment",
    );
  });
  ...
  it("handles deeply nested content and many backtick runs without overflowing", () => {
    expect(
      htmlToMarkdown(
        `${"<div>".repeat(2_000)}Visible<span hidden>secret</span>${"</div>".repeat(2_000)}`,
      ),
    ).toBe("Visible");
  ...
  it("preserves paragraph boundaries across many sibling elements", () => {
    const count = 30_000;
    expect(htmlToMarkdown("<p>x</p>".repeat(count))).toBe(
      Array<string>(count).fill("x").join("\n\n"),
    );
  });
