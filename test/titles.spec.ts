...
describe("the AI binding under the test pool", () => {
  it("exists but cannot be run", async () => {
    ...
    await expect(
      env.AI.run("@cf/ibm-granite/granite-4.0-h-micro", {
...
describe("automatic titling on POST /api/documents", () => {
  ...
  it("uses a supplied title verbatim, over both the heading and the file name", async () => {
    const doc = await createDoc(
      uploadBody("# Heading Not Used", { name: "file.md", title: "Explicit Title" }),
    );
...
describe("the final title fallback", () => {
  it("never yields a blank title, whatever the caller's fallback", async () => {
    for (const fallback of ["", "   "]) {
      expect(
        await resolveNewTitle(env, { fallback, kind: "md", source: "prose with no heading" }),
      ).toBe("untitled");
      // HTML uses the caller fallback directly and needs the same blank check.
      expect(await resolveNewTitle(env, { fallback, kind: "html", source: "<p>x</p>" })).toBe(
        "untitled",
      );
    }
  });
  ...
  it("refuses a blank heading and uses the caller's fallback", async () => {
    ...
    // name does.
    expect(
      await resolveNewTitle(env, { fallback: "notes.md", kind: "md", source: "#   \n\nbody" }),
    ).toBe("notes.md");
  ...
  it("trims both the fallback it ends on and the heading it accepts", async () => {
    expect(
      await resolveNewTitle(env, { fallback: "  notes.md  ", kind: "md", source: "no heading" }),
    ).toBe("notes.md");
    expect(
      await resolveNewTitle(env, {
        fallback: "notes.md",
        kind: "md",
        source: "#  Padded Heading  \n\nbody",
      }),
    ).toBe("Padded Heading");
  });
});
...
describe("automatic titling is create-only", () => {
  it("keeps the original title on a new version whose heading says otherwise", async () => {
    ...
    const v2 = await addVersion(
      doc.id,
      uploadBody("# Completely Different Heading", { name: "v2.md" }),
    );
