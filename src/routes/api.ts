...
async function readUpload(
  ...
  if (paths.length && paths.length !== uploaded.length)
    return c.json({ error: "Each file needs one path." }, 400);
  if (
    (!uploaded.length && (!update || !removed.length)) ||
    uploaded.some((file) => !(file instanceof File))
  )
    return c.json({ error: "file is required" }, 400);
  if (
    paths.some((path) => typeof path !== "string") ||
    removed.some((path) => typeof path !== "string")
  )
...
apiRoutes.get("/documents/:id/content", async (c) => {
  ...
  const obj = await readVersionContent(
    c.env,
    id,
    asked,
    nowSeconds(),
...
apiRoutes.post("/documents/:id/shares", async (c): Promise<Response> => {
  ...
  if (issued.ok) {
    const { share } = issued;
    return c.json(
      { token: share.token, expires_at: share.expires_at, url: `/v/${share.token}` },
      201,
    );
  }
