...
const SOURCE_FIELDS = {
  ...
  media_type: z
    .string()
    .optional()
    .describe("Media type for a file without a recognized extension."),
};
const INPUT_FILE = z.object({
  path: z
    .string()
    .describe("Relative path within the document, including directories for relative links."),
...
const FILES = z
  ...
  .optional()
  .describe(
    "Files to add or replace by path. Use this instead of top-level content for a multi-file document.",
  );
...
function decodeFile(file: z.infer<typeof INPUT_FILE>) {
  ...
  if (source === null)
    throw new Error(`Content for ${JSON.stringify(file.path)} is not valid base64.`);
  if (sourceBytes(source) > MAX_BYTES)
    throw new Error(`Content exceeds the ${MAX_BYTES}-byte limit.`);
...
function missing(id: string): CallToolResult {
  return failure(
    `No live document with id ${id}. It may never have existed, or its TTL has passed.`,
  );
}
...
function tooLarge(source: string | ArrayBuffer): CallToolResult | null {
  return sourceBytes(source) > MAX_BYTES
    ? failure(`Content exceeds the ${MAX_BYTES}-byte limit.`)
    : null;
}
...
async function readCapped(
  ...
  function append(decoded: string): boolean {
    ...
    content += new TextDecoder("utf-8", { fatal: false, ignoreBOM: true }).decode(
...
function buildServer(c: Context<{ Bindings: Env }>): McpServer {
    ...
    "cat",
    {
      ...
      inputSchema: {
        ...
        file: z
          ...
          .optional()
          .describe(
            "File path within the document (default: first file). Use files to list paths.",
          ),
        raw: z
          .boolean()
          .default(false)
    ...
    "files",
    {
      ...
      inputSchema: {
        ...
        version: z
          ...
          .min(1)
          .optional()
          .describe("Version to inspect (default: current)."),
      },
    },
    async ({ id, version }) => {
  ...
  server.registerTool(
    "push",
    {
      ...
      inputSchema: {
        measure: z
          .boolean()
          .optional()
          .describe("Include upload timings in the result and Worker logs."),
        content: z
          .string()
          .optional()
        ...
        share: z
          ...
          .default(false)
          .describe(
            "Create a public /v/{token} link only when the user explicitly requests sharing.",
          ),
    ...
    async ({
      ...
      if ((content === undefined) === (files === undefined))
        return failure("Supply either content or files.");
      if (files !== undefined && files.length === 0)
        return failure("At least one file is required.");
            ...
            ? await resolveNewTitle(c.env, {
                ...
                source:
                  typeof titleSource === "string"
                    ? titleSource
                    : new TextDecoder().decode(titleSource),
              })
  ...
  server.registerTool(
    "rename",
    {
      ...
      inputSchema: {
        ...
        expected_state: z
          .string()
          .describe(
            "Opaque STATE token from ls or expected_state from suggest_title. Copy it exactly.",
          ),
      },
    },
    async ({ id, title, expected_state }) => {
      try {
        ...
        if (err instanceof TitleOperationError)
          return err.status === 404 ? missing(id) : failure(err.message);
        throw err;
      }
    },
  );
  ...
  server.registerTool(
    "revoke",
    {
      ...
      inputSchema: {
        token: z.string().describe("Share token to revoke (the s_… value, not a document id)."),
      },
    },
    async ({ token }) => {
    ...
    "rollback",
    {
      ...
      inputSchema: {
        ...
        version: z
          ...
          .int()
          .min(1)
          .describe("Version number to restore (see the `versions` tool)."),
      },
    },
    async ({ id, version }) => {
      const now = nowSeconds();
      ...
      return text(
        `Rolled back ${id} to v${result.current_version}. Live share links serve it from their next load.`,
      );
    },
  );
    ...
    "update",
    {
      ...
      inputSchema: {
        ...
        title: z
          .string()
          .optional()
          .describe("New document title (default: keep the current one)."),
      },
    },
    async ({
      content,
      ...
      if (!bundle && delete_paths?.length)
        return failure("Use files instead of content when deleting paths.");
      content ??= "";
      if (encoding === "base64" && content.length > 4 * Math.ceil(MAX_BYTES / 3)) {
        return failure(`Content exceeds the ${MAX_BYTES}-byte limit.`);
      }
      ...
      const now = nowSeconds();
      const snapshot = await measureUpload(timing, "d1_snapshot", () =>
        getLiveUpdateSnapshot(c.env.DB, id, now),
      );
