...
rawRoutes.on("GET", ["/:token", "/:token/*"], async (c) => {
  ...
  try {
    path = pathname.startsWith(prefix)
  ...
  if (
    /^(image\/(png|jpeg|gif|webp|avif|x-icon)|audio\/(mpeg|wav|ogg|mp4)|video\/(mp4|webm))$/.test(
      file.media_type,
    )
  ) {
    return new Response(obj.body, { headers: { "Content-Type": file.media_type } });
  ...
  return new Response(wrapViewerHtml(title, body), {
    headers: { "Content-Type": "text/html; charset=utf-8" },
  });
});
