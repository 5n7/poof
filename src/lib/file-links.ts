...
export function resolveFileLink(
  ...
  try {
    const base = new URL(
      `https://document.invalid/${currentPath.split("/").map(encodeURIComponent).join("/")}`,
    );
...
export function linkDocumentFiles(
  response: Response,
  token: string,
  path: string,
  paths: string[],
        ...
        for (const attribute of ["href", "src", "poster", "data"]) {
          ...
          if (target)
            element.setAttribute(
              attribute,
              rawFileUrl(token, target.path) + target.search + target.hash,
            );
        }
      },
    })
