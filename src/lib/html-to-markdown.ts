...
function textContent(node: Node): string {
  ...
  while (pending.length) {
    ...
    if (current.nodeName === "#text")
      text.push((current as DefaultTreeAdapterTypes.TextNode).value);
...
export function htmlToMarkdown(source: string): string {
  ...
  function list(element: Element): string {
    ...
    const items = element.childNodes
      .filter(isElement)
      .filter((node) => node.tagName === "li" && !isHidden(node));
    return block(
      literal(
        items
          .map((item) => {
  ...
  function table(element: Element): string {
    ...
    const cells = rows.map((row) =>
      row.childNodes
        .filter(isElement)
        .filter((cell) => ["th", "td"].includes(cell.tagName) && !isHidden(cell)),
    );
    const values = cells.map((row) =>
      row.map((cell) =>
        children(cell)
          .trim()
          .replace(literalPattern, (_, index: string) =>
            literals[Number(index)].replace(/\|/g, "\\|"),
          )
          .replace(/\n+/g, "<br>"),
      ),
    );
    ...
    return (
      ...
      block(
        literal(
          [line(heading), line(Array<string>(width).fill("---")), ...values.map(line)].join("\n"),
        ),
      )
    );
  }
  ...
  function renderNode(node: Node): string {
    ...
    if (tag === "pre") {
      ...
      const language =
        classes.match(/(?:^|\s)language-([\w+-]+)/)?.[1] ??
        (/(?:^|\s)mermaid(?:\s|$)/.test(classes) ? "mermaid" : "");
      const content = textContent(node);
      const fence = fenceFor(content, 3);
      return block(
        literal(`${fence}${language}\n${content}${content.endsWith("\n") ? "" : "\n"}${fence}`),
      );
    }
    ...
    if (tag === "blockquote")
      return block(literal(restoreLiterals(content.trim()).replace(/^/gm, "> ")));
