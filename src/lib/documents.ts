...
function validateFiles(files: NewFileInput[]): void {
  if (files.length > MAX_FILES)
    throw new DocumentInputError(`A document supports at most ${MAX_FILES} files.`);
  ...
  if (bytes > MAX_BYTES)
    throw new DocumentInputError(
      `Document upload is ${bytes} bytes, over the ${MAX_BYTES}-byte limit.`,
      413,
    );
}
...
export async function createDocument(
  ...
  try {
    await measureUpload(timing, "r2_put", () =>
      mapConcurrent(files, BLOB_CONCURRENCY, (file, index) =>
        env.BLOBS.put(rows[index]!.r2_key, file.source),
      ),
    );
    if (
      !(await measureUpload(timing, "d1_publish", () =>
        applyNewVersion(env.DB, id, 1, now, input.title, 0),
      ))
    )
...
export async function addVersion(
  ...
  const previous =
    previousFiles ??
    (await measureUpload(timing, "d1_files", () => listVersionFiles(env.DB, doc.id, doc.version)));
  ...
  const files = input.files ?? [
    {
      ...
      media_type:
        input.media_type ??
        (input.kind === doc.kind ? doc.media_type : defaultMediaType(input.kind)),
      path: input.filename
  ...
  for (const path of input.delete_paths ?? []) {
    ...
    if (!previous.some((file) => file.path === path))
      throw new DocumentInputError(`File does not exist: ${path}`);
    if (files.some((file) => file.path === path))
      throw new DocumentInputError(`Cannot upload and delete the same path: ${path}`);
    deleted.add(path);
  }
  if (!files.length && !deleted.size)
    throw new DocumentInputError("Upload or delete at least one file.");
  const retained =
    legacy && previous.length === 1 ? [] : previous.filter((file) => !deleted.has(file.path));
  ...
  try {
    await measureUpload(timing, "r2_put", () =>
      mapConcurrent(writes, BLOB_CONCURRENCY, ({ row, source }) =>
        env.BLOBS.put(row.r2_key, source),
      ),
    );
    if (
      !(await measureUpload(timing, "d1_publish", () =>
...
export async function rollbackDocument(
  ...
  const sources = await mapConcurrent(files, BLOB_CONCURRENCY, (file) =>
    env.BLOBS.head(file.r2_key),
  );
...
export type IssueShareResult =
  ...
  | { ok: false; reason: "invalid-ttl" | "not-found" };
...
export async function readVersionContent(
  ...
  if (!obj) return null;
  if (raw || file.kind !== "html")
    return { ...metadata, source_size: obj.size, body: obj.body, size: obj.size };
...
export async function documentTitleState(doc: {
  id: string;
  title: string;
  current_version: number;
}): Promise<string> {
...
export async function documentTitleMetadata(
  doc: Pick<DocumentRow, "id" | "title" | "current_version" | "updated_at">,
) {
  return {
    id: doc.id,
    title: doc.title,
...
export function normalizeDocumentTitle(value: unknown): string {
  ...
  if (/\p{Cs}/u.test(value))
    throw new TitleOperationError("Title contains invalid Unicode characters.", 400);
...
async function titleSnapshot(
  ...
  if (
    !expected ||
    typeof expected.expected_state !== "string" ||
    !/^[a-f0-9]{64}$/.test(expected.expected_state)
  )
...
async function documentTitleExcerpt(
  ...
  if (!files.length)
    throw new TitleOperationError(
      "This document has no readable text to suggest a title from.",
      422,
    );
