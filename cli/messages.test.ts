...
describe("authentication command messages", () => {
  test("login and logout state that service authentication remains selected", () => {
    ...
    expect(logoutMessage("service", "https://poof.example", true)).toContain(
      "service authentication remains active",
    );
    expect(logoutMessage("service", "https://poof.example", false)).toContain(
      "service authentication remains active",
    );
