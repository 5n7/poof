...
describe("loadConfig", () => {
  ...
  test("rejects a partial service pair and a missing URL", () => {
    expect(() => loadConfig({ POOF_URL: URL, POOF_ACCESS_CLIENT_ID: "id" })).toThrow(
...
function runtime(responses: Response[]): {
  value: ApiRuntime;
  requests: Request[];
  refreshes: boolean[];
} {
...
const service: PoofConfig = {
  url: URL,
  auth: { type: "service", clientId: "id", clientSecret: "secret" },
};
...
test("OAuth writes refresh after a 401 but are never replayed", async () => {
  ...
  await expect(api(oauth, "POST", "/api/documents", { title: "x" }, r.value)).rejects.toThrow(
    "rerun the command",
  );
...
test("authentication diagnostics name the selected method", async () => {
  for (const [config, status, expected] of [
    ...
    await expect(api(config, "GET", "/api/documents", undefined, r.value)).rejects.toThrow(
      expected,
    );
  }
});
