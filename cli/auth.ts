...
function parseCredential(raw: string | null, resource: string): StoredCredential | null {
  ...
  try {
    ...
    throw new Error(
      "Stored OAuth credential is corrupt. Run 'poof auth logout', then 'poof auth login'.",
    );
  }
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
    throw new Error(
      "Stored OAuth credential is invalid. Run 'poof auth logout', then 'poof auth login'.",
    );
  }
...
async function readRawCredential(resource: string, runtime: AuthRuntime): Promise<string | null> {
  try {
    ...
    throw new Error(
      `Cannot read OAuth credential from the OS credential manager: ${(error as Error).message}`,
      {
        cause: error,
      },
    );
...
async function saveCredential(value: StoredCredential, runtime: AuthRuntime): Promise<void> {
  try {
    await runtime.secrets.set(
      TOKEN_SERVICE,
      resourceKey(value.registration.resource),
      JSON.stringify(value),
    );
  } catch (error) {
    throw new Error(
      `Cannot save OAuth credential in the OS credential manager: ${(error as Error).message}`,
      {
        cause: error,
      },
    );
...
async function deleteCredential(resource: string, runtime: AuthRuntime): Promise<boolean> {
  try {
    ...
    throw new Error(
      `Cannot delete OAuth credential from the OS credential manager: ${(error as Error).message}`,
      {
        cause: error,
      },
    );
  ...
  for (;;) {
    ...
    if (workError && releaseError)
      throw new AggregateError(
        ...
        "OAuth credential mutation and lock release both failed.",
      );
...
export async function oauthAccessToken(
  ...
  return withCredentialLock(resource, runtime, async () => {
    ...
    if (!reread || !sameOAuthRegistration(reread.registration, registered)) {
      throw new Error(
        "OAuth registration changed while waiting for the credential lock. Retry the command.",
      );
    }
    if (!reread.tokens) throw new Error(`Not logged in to ${resource}. Run 'poof auth login'.`);
    if (forceRefresh && reread.tokens.accessToken !== current.accessToken)
      return reread.tokens.accessToken;
    ...
    try {
      ...
      if (/\binvalid_grant\b/.test((error as Error).message)) {
        await saveCredential(
          { version: CREDENTIAL_VERSION, registration: reread.registration },
          runtime,
        );
        throw new Error(`OAuth login for ${resource} has expired. Run 'poof auth login'.`, {
          cause: error,
        });
      }
      throw error;
...
export async function loginOAuth(
  ...
  const callbackPath =
    reusedRegistration?.callbackPath ?? `/oauth/callback/${runtime.randomUrlSafe(18)}`;
  ...
  try {
    if (reusedRegistration && !sameOAuthDiscovery(reusedRegistration, discovery)) {
      throw new Error(
        "Stored OAuth registration no longer matches discovery. Run 'poof auth login --new-client'.",
      );
    }
    const registration =
      reusedRegistration ?? (await registerClient(discovery, listener.port, callbackPath, runtime));
    ...
    const replaced = await withCredentialLock(resource, runtime, async () => {
      ...
      if (baseline) {
        if (!reread || !sameOAuthRegistration(reread.registration, baseline.registration)) {
          throw new Error(
            "OAuth registration changed during browser authorization. Retry 'poof auth login'.",
          );
        }
      } else if (reread) {
      ...
      return reread?.tokens && reread.registration.clientId !== registration.clientId
        ? reread
        : null;
    });
...
export async function logoutOAuth(
  ...
  return withCredentialLock(resource, runtime, async () => {
      ...
      try {
        ...
        return {
          ...
          revocationError: new AggregateError(
            [error, deleteError],
            "OAuth credential could not be read or deleted.",
          ),
        };
      }
    ...
    try {
      await saveCredential(
        { version: CREDENTIAL_VERSION, registration: credential.registration },
        runtime,
      );
    } catch (error) {
