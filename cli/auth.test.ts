  ...
  const fetcher = (async (input: string | URL | Request, init?: RequestInit) => {
    ...
    if (request.url === `${RESOURCE}/`) {
      return new Response(null, {
        status: 401,
        headers: {
          "WWW-Authenticate": `Bearer resource_metadata="${RESOURCE}/.well-known/resource"`,
        },
      });
    }
  ...
  const runtime: AuthRuntime = {
    ...
    listen: ({ port, timeoutMs }) => {
      ...
      return {
        ...
        wait: () =>
          Promise.resolve({ type: "success", code: "authorization-code", issuer: ISSUER }),
        close: () => {
          if (closed) return;
          closed = true;
          events.push("close");
...
describe("OAuth persistence", () => {
  ...
  test("strictly validates the encrypted registration binding", async () => {
    ...
    for (const [field, value] of [
      ...
      await expect(oauthAccessToken(RESOURCE, false, f.runtime)).rejects.toThrow(
        "Stored OAuth credential is invalid",
      );
    }
  });
  ...
  test("allows token rotation under the same registration before the final login commit", async () => {
    ...
    const loggingIn = loginOAuth(
      RESOURCE,
      { newClient: false, onAuthorization: () => ready.resolve() },
      f.runtime,
    );
    await ready.promise;
  ...
  test("reports old-grant revocation failure after committing a replacement client", async () => {
    ...
    const result = await loginOAuth(
      RESOURCE,
      { newClient: true, onAuthorization: () => undefined },
      f.runtime,
    );
