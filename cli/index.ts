...
function uploadTiming(): ApiTiming | undefined {
  return process.env.POOF_TIMING === "1"
    ? { authMs: 0, httpMs: 0, serverTiming: null, traceId: null }
    : undefined;
}
...
function requireVersion(version: string): void {
  if (!/^[1-9][0-9]*$/.test(version))
    fail(`invalid version '${version}' (expected a positive integer)`);
}
...
function printTable(header: string[], rows: string[][]): void {
  ...
  const widths = header.map((h, i) =>
    Math.max(Bun.stringWidth(h), ...rows.map((r) => Bun.stringWidth(r[i]))),
  );
...
const cat = defineCommand({
  meta: {
    name: "cat",
    description:
      "Print source text or readable Markdown for HTML. Use --raw to download original bytes.",
  },
  args: {
    "doc-id": {
      type: "positional",
  ...
  run: ({ args }) =>
    attempt(async () => {
      ...
      // normal stop.
      await pipeline(Readable.fromWeb(body), process.stdout, { end: false }).catch(
  ...
  run: ({ args }) =>
    attempt(async () => {
      if (args.open !== false && !process.stdin.isTTY) {
        throw new Error(
          "login requires a terminal; pass --no-open to print the authorization URL explicitly",
        );
      }
      const cfg = loadConfig();
  ...
  run: ({ args }) =>
    attempt(async () => {
      ...
      try {
        ...
        const doc = await api<DocumentRow>(
          cfg,
          "POST",
          "/api/documents",
          form,
        ...
        if (args.share) {
          ...
          const share = await api<ShareResult>(
            cfg,
            "POST",
            p`/api/documents/${doc.id}/shares`,
            body,
          );
          progress.stop();
...
const rename = defineCommand({
  ...
  run: ({ args }) =>
    attempt(async () => {
      ...
      const state =
        args["expected-state"] ??
        (await api<TitleSnapshot>(cfg, "GET", p`/api/documents/${args["doc-id"]}`)).state;
      const result = await api<TitleSnapshot>(
        cfg,
        "PATCH",
        p`/api/documents/${args["doc-id"]}/title`,
        {
          title: args.title,
...
const share = defineCommand({
  ...
  run: ({ args }) =>
    attempt(async () => {
      const cfg = loadConfig();
      const body = args["share-ttl"] ? { ttl: args["share-ttl"] } : {};
      const result = await api<ShareResult>(
        cfg,
        "POST",
        p`/api/documents/${args["doc-id"]}/shares`,
        body,
      );
...
const suggestTitle = defineCommand({
  ...
  args: {
    "doc-id": {
      type: "positional",
      description: "Document id to suggest a title for.",
      required: true,
    },
  },
  run: ({ args }) =>
    attempt(async () => {
      ...
      const result = await api<TitleSuggestion>(
        cfg,
        "POST",
        p`/api/documents/${id}/title-suggestion`,
        {
...
const update = defineCommand({
  meta: {
    name: "update",
    description:
      "Merge uploaded files by path into a new version, keeping other files and all share links.",
  },
  args: {
    "doc-id": {
      type: "positional",
  ...
  run: ({ args, rawArgs }) =>
    attempt(async () => {
      ...
      try {
        ...
        const filesMs = performance.now() - started;
        if (!upload.files.length && !removed.length)
          throw new Error("provide files to upload or --delete paths");
        progress.update("Preparing upload");
...
const versions = defineCommand({
  ...
  run: ({ args }) =>
    attempt(async () => {
      const cfg = loadConfig();
      const result = await api<VersionsResult>(
        cfg,
        "GET",
        p`/api/documents/${args["doc-id"]}/versions`,
      );
