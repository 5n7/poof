...
function httpsUrl(value: unknown, label: string, origin?: string): string {
  ...
  if (url.username || url.password || url.hash)
    throw new Error(`${label} contains forbidden URL components.`);
  if (origin && url.origin !== origin)
    throw new Error(`${label} must use the OAuth issuer origin.`);
  return url.href;
}
...
export function canonicalResource(raw: string): string {
  ...
  if (
    ...
    throw new Error(
      "POOF_URL must contain only an HTTPS origin, without credentials, a path, query, or fragment.",
    );
  }
  return url.origin;
}
...
export function validateOAuthRegistration(
  ...
  if (
    ...
    !(
      typeof registration.revocationEndpoint === "string" ||
      registration.revocationEndpoint === null
    ) ||
...
function resourceMetadataUrl(header: string | null, resource: string): string {
  if (!header || !/^Bearer\b/i.test(header.trim())) {
    throw new Error(
      "Cloudflare Access did not advertise Managed OAuth. Enable it on the owner application.",
    );
  }
  const match = /(?:^|,)\s*resource_metadata\s*=\s*"([^"]+)"/i.exec(
  ...
  if (new URL(url).origin !== resource)
    throw new Error("OAuth resource metadata must be served by POOF_URL.");
  return url;
}
...
export async function discoverOAuth(
  ...
  if (advertisedIssuer !== issuer)
    throw new Error("OAuth authorization server metadata has an issuer mismatch.");
  ...
  if (
    !requireStringArray(server.code_challenge_methods_supported, "OAuth PKCE methods").includes(
      "S256",
    )
  ) {
  ...
  if (
    !requireStringArray(server.response_types_supported, "OAuth response types").includes("code")
  ) {
...
export async function registerClient(
  ...
  if (body.token_endpoint_auth_method !== "none")
    throw new Error("OAuth server did not register a public client.");
  if (body.client_secret !== undefined)
    throw new Error("OAuth server returned a client secret for a public client.");
...
export function pkce(runtime: Pick<OAuthRuntime, "randomUrlSafe">): {
  verifier: string;
  challenge: string;
} {
  for (;;) {
...
export function oauthProtocolError(
  ...
  const code =
    typeof codeValue === "string" ? codeValue.replace(/[^A-Za-z0-9_.-]/g, "").slice(0, 64) : "";
  const description =
    typeof descriptionValue === "string"
      ? descriptionValue.replace(/[\p{Cc}\p{Cf}]/gu, "").slice(0, 240)
      : "";
...
function readTokens(
  ...
  const refreshToken =
    body.refresh_token === undefined
      ...
      : requireString(body.refresh_token, "OAuth refresh token");
  ...
  if (body.resource !== undefined) {
    ...
    if (returnedResource !== resource)
      throw new Error("OAuth token response names a different resource.");
  }
...
export async function exchangeCode(
  ...
  return readTokens(
    await tokenJson(res, "OAuth code exchange"),
    runtime.now(),
...
export async function revokeGrant(
  ...
  if (!registration.revocationEndpoint)
    throw new Error("OAuth server does not advertise token revocation.");
