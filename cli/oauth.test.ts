  ...
  return (async (input: string | URL | Request) => {
    ...
    if (url === `${RESOURCE}/`) {
      return new Response(null, {
        status: 401,
        headers: {
          "WWW-Authenticate": `Bearer realm="OAuth", resource_metadata="${RESOURCE}/.well-known/resource"`,
        },
      });
    }
...
describe("canonicalResource", () => {
  ...
  test("rejects insecure URLs and URL components", () => {
    for (const value of [
      "http://poof.example",
      "https://poof.example/api",
      "https://user@poof.example",
      "nope",
    ]) {
...
describe("registration validation and comparison", () => {
  test("validates persisted registrations with the discovery URL rules", () => {
    ...
    expect(
      validateOAuthRegistration(
        { ...REGISTRATION, tokenEndpoint: "https://evil.example/token" },
        RESOURCE,
      ),
    ).toBeNull();
    expect(
      validateOAuthRegistration({ ...REGISTRATION, callbackPath: "/other" }, RESOURCE),
    ).toBeNull();
  });
  ...
  test("compares discovery fields before registration identity", () => {
    ...
    expect(
      sameOAuthDiscovery(REGISTRATION, { ...DISCOVERY, tokenEndpoint: `${ISSUER}/other` }),
    ).toBe(false);
...
describe("discovery", () => {
  ...
  test("rejects cross-origin resource metadata", async () => {
    ...
    await expect(discoverOAuth(RESOURCE, { fetch: fetcher })).rejects.toThrow(
      "must be served by POOF_URL",
    );
  });
    ...
    await expect(
      discoverOAuth(RESOURCE, {
        fetch: discoveryFetch({
          server: { token_endpoint_auth_methods_supported: ["client_secret_basic"] },
        }),
      }),
...
test("dynamic registration pins the resource and exact loopback callback", async () => {
  ...
  const registered = await registerClient(DISCOVERY, 45678, "/oauth/callback/random", {
    fetch: fetcher,
  });
...
test("dynamic registration rejects a client secret", async () => {
  ...
  await expect(
    registerClient(DISCOVERY, 45678, "/oauth/callback/random", { fetch: fetcher }),
...
describe("tokens", () => {
  test("exchanges and refreshes without losing an unrotated refresh token", async () => {
    ...
    const first = await exchangeCode(
      REGISTRATION,
      "http://127.0.0.1:45678/oauth/callback/random",
      "code",
      "verifier",
      {
        fetch: fetcher,
...
describe("callback validation", () => {
  ...
  const callback = (
    query: string,
    overrides: { authority?: string; method?: string; path?: string } = {},
  ) =>
    parseCallbackRequest(
      new Request(
  ...
  test("accepts only the exact authority, method, path, and state", () => {
    ...
    expect(
      callback("state=state&code=no", { authority: `localhost:${config.port}` }).response.status,
    ).toBe(404);
    expect(callback("state=state&code=no", { authority: "127.0.0.1:49124" }).response.status).toBe(
      404,
    );
  ...
  test("delegates fresh port 0 and validates against the assigned server port", async () => {
    ...
    const response = options?.fetch(
      new Request("http://127.0.0.1:52345/oauth/callback/exact?state=state&code=yes"),
    );
