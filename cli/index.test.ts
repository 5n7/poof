...
test("cat combines raw and version options and streams without adding bytes", async () => {
  ...
  try {
    ...
    const result = await run(
      ["cat", "doc", "--raw", "--version", "2", "--file", "adr/001.md"],
      environment,
    );
    expect(result.exitCode).toBe(0);
...
test("multi-file uploads preserve relative paths and update supports explicit deletions", async () => {
  ...
  try {
    ...
    let result = await run(
      ["push", join(directory, "overview.md"), join(directory, "adr", "001.html")],
      environment,
    );
    ...
    result = await run(
      ["update", "doc", "--delete", "old.md", "--delete", "unused.html"],
      environment,
    );
    expect(result.exitCode, result.stderr).toBe(0);
    ...
    result = await run(
      ["update", "doc", "--delete=old.md", "--", join(directory, "overview.md")],
      environment,
    );
    expect(result.exitCode, result.stderr).toBe(0);
...
test("update does not treat title or root values as deletion options", async () => {
  ...
  try {
    ...
    let result = await run(
      ["update", "doc", "--title", "--delete=keep.md", "--delete", "remove.md"],
      environment,
    );
    expect(result.exitCode, result.stderr).toBe(0);
  ...
  const server = Bun.serve({
    ...
    fetch(request) {
      ...
      return Response.json({
        files: [{ path: "adr/001.md", kind: "md", media_type: "text/markdown" }],
      });
    },
  });
  try {
...
test("rename reads a snapshot and changes only title metadata", async () => {
  ...
  const server = Bun.serve({
    ...
    async fetch(request) {
      ...
      return Response.json(
        request.method === "GET" ? snapshot : { ...snapshot, title: "New title", updated_at: 200 },
      );
    },
  });
  try {
...
test("rename preserves a reviewed suggestion's preconditions and does not retry conflicts", async () => {
  ...
  const server = Bun.serve({
    ...
    async fetch(request) {
      ...
      return Response.json(
        { error: "Document changed. Review the current title and try again." },
        { status: 409 },
      );
    },
  });
  try {
    const result = await run(
      ["rename", "doc", "--title", "Reviewed title", "--expected-state", "a".repeat(64)],
      {
        POOF_URL: `http://127.0.0.1:${server.port}`,
        POOF_ACCESS_CLIENT_ID: "test",
        POOF_ACCESS_CLIENT_SECRET: "test",
    ...
    expect(requests).toEqual([
      { method: "PATCH", body: { title: "Reviewed title", expected_state: "a".repeat(64) } },
    ]);
  } finally {
    server.stop(true);
  }
  ...
  const server = Bun.serve({
    ...
    fetch(request) {
      return request.method === "GET"
        ...
        : Response.json(
            { error: "AI title generation is unavailable. Try again or enter a title." },
            { status: 503 },
          );
    },
  });
