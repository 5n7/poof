...
export async function collectFiles(
  ...
  if (!root && entries.length > 1) {
    for (const path of paths) {
      while (relative(base, path).startsWith(`..${sep}`) || relative(base, path) === "..")
        base = dirname(base);
    }
  }
  ...
  async function visit(path: string): Promise<void> {
    ...
    if (files.length >= MAX_FILES)
      throw new Error(`A document supports at most ${MAX_FILES} files.`);
    if (bytes + stat.size > MAX_BYTES)
      throw new Error(`Document upload exceeds the ${MAX_BYTES}-byte limit.`);
  ...
  return {
    files,
    explicitPaths:
      root !== undefined || inputs.length > 1 || entries.some((entry) => entry.stat.isDirectory()),
  };
}

export function appendFiles(
  form: FormData,
  upload: Awaited<ReturnType<typeof collectFiles>>,
): void {
...
export function deletionPaths(rawArgs: string[]): string[] {
  ...
  for (const token of tokens) {
    ...
    if (!token.value || (!token.inlineValue && token.value.startsWith("-")))
      throw new Error(
        "--delete requires a non-empty path. Use --delete=<path> for paths starting with '-'.",
      );
